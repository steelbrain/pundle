...
var __sb_pundle_hot = function () {
  function __sb_pundle_hot() {
    _classCallCheck(this, __sb_pundle_hot);
  ...
  _createClass(__sb_pundle_hot, [{
    key: 'accept',
    value: function accept(a, b) {
      var clause = typeof a === 'string' ? a : '*';
      var callback = typeof a === 'function' && a || typeof b === 'function' && b || null;

      this.accepts.add(clause);
      if (callback) {
        this.callbacks_accept.add({ clause: clause, callback: callback });
      }
    }
  }, {
    key: 'decline',
    ...
    key: 'addDisposeHandler',
    value: function addDisposeHandler(callback) {
      this.callbacks_dispose.add(callback);
    }
  }, {
    key: 'removeDisposeHandler',
    value: function removeDisposeHandler(callback) {
      this.callbacks_dispose.delete(callback);
    }
  }]);

  return __sb_pundle_hot;
}();

function __sb_pundle_hmr_is_accepted(id, givenMatchAgainst) {
  var module = __sb_pundle.cache[id];
  var matchAgainst = givenMatchAgainst || id;
  return module && ((module.hot.accepts.has('*') || module.hot.accepts.has(matchAgainst)) && 1 || module.parents.some(function (entry) {
    return __sb_pundle_hmr_is_accepted(entry, matchAgainst);
  }) && 2);
}

function __sb_pundle_hmr_get_update_order(applyTo) {
  var unresolved = [].concat(applyTo);
  var resolved = [];
  while (unresolved.length) {
    var i = unresolved.length;
    var passed = true;
    var foundOne = false;
    while (i--) {
      var _id = unresolved[i];
      var module = __sb_pundle.cache[_id];
      var acceptanceStatus = __sb_pundle_hmr_is_accepted(_id);
      if (!module || applyTo.indexOf(_id) !== -1 && !acceptanceStatus) {
        passed = false;
      }
      var itemResolved = !module.parents.length || module.parents.every(function (parent) {
      ...
      if (itemResolved) {
        ...
        for (var j = 0, length = module.parents.length; j < length; ++j) {
          var parent = module.parents[j];
    ...
    if (!passed || !foundOne) {
      var error = new Error('Unable to apply HMR. Page refresh will be required');
...
function __sb_pundle_hmr_apply(applyTo) {
  var modules = __sb_pundle_hmr_get_update_order(applyTo);

  var _loop = function _loop(i, length) {
    var id = modules[i];
    var module = __sb_pundle.cache[id];
    var data = {};
    var oldHot = module.hot;
    ...
    oldHot.callbacks_accept.forEach(function (_ref) {
      var clause = _ref.clause;
